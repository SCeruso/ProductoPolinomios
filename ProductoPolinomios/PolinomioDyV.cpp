...
Polinomio PolinomioDyV::desplazar(Polinomio* p, int n){
	...
	res.setPolinomio(v, p->get_terminos() + n);
	delete v;
	return res;
}
void  PolinomioDyV::divide(Polinomio* p, Polinomio* q, Polinomio &qh, Polinomio &ql, Polinomio &ph, Polinomio &pl) {
	...
	qh.setPolinomio(vqh, p->get_terminos() - mp);
	ql.setPolinomio(vql, mp);
	ph.setPolinomio(vph, p->get_terminos() - mp);
	pl.setPolinomio(vpl, mp);
